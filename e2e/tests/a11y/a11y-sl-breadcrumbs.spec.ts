...
test.describe('sl-breadcrumbs accessibility', () => {
  ...
  test(`should allow keyboard operability of home link`, async ({ page }) => {
    ...
    const urlPromise = page.waitForURL(baseUrl);
  ...
  test(`should allow keyboard operability of current page link`, async ({
    page,
  }) => {
  ...
  test('should have aria-current attribute on the current page link', async ({
    page,
  }) => {
  ...
  test('should have aria-label when first link is icon-only', async ({
    page,
  }) => {
