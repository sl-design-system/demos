...
test.describe('sl-tab-group', () => {
  ...
  test('should display content of first tab after clicking first tab', async ({
    ...
    await expect(
      page
        .getByRole('tabpanel')
        .filter({ has: page.getByText(PANELS.general) }),
    ).toBeVisible();
    await expect(
      page.locator('sl-button', { hasText: 'Action' }),
    ).toBeVisible();
  });
  ...
  test('should change content in tab panel after clicking active tabs', async ({
    ...
    await expect(
      page
        .getByRole('tabpanel')
        .filter({ has: page.getByText(PANELS.settings) }),
    ).toBeVisible();
  });
  ...
  test('should not display content of disabled tab after clicking disabled tab', async ({
    ...
    await expect(
      page.getByRole('tab', { name: 'Disabled' }),
    ).not.toHaveAttribute('selected');
    await expect(
      page
        .getByRole('tabpanel')
        .filter({ has: page.getByText(PANELS.disabled) }),
    ).toBeHidden();
  });
});
