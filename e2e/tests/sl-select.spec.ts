...
test.describe('sl-select', () => {
  ...
  test('should expand listbox on click', async ({ page }) => {
    await page
      .getByRole('combobox')
  ...
  test('should allow to click option in listbox', async ({ page }) => {
    await page
      .getByRole('combobox')
      .filter({ hasText: 'Select an option' })
      .click();
    ...
    await expect(
      page.getByRole('combobox').filter({ hasText: 'Option 1' }),
    ).toBeVisible();
  });

  test('should change selected option on click', async ({ page }) => {
    await page
      .getByRole('combobox')
      .filter({ hasText: 'Select an option' })
      .click();
    ...
    await expect(
      page.getByRole('combobox').filter({ hasText: 'Option 2' }),
    ).toBeVisible();
  });
  ...
  test('should collapse listbox after click outside listbox', async ({
    ...
    await expect(
      page.getByRole('combobox').filter({ hasText: 'Select an option' }),
    ).toBeVisible();
    await expect(
  ...
  test('should show validation message after clicking report validity button', async ({
    ...
    await expect(
      page.getByText('Please choose an option from the list.', { exact: true }),
    ).toBeVisible();
  });
  ...
  test('should not allow to click options in disabled select', async ({
    ...
    const disabledSelect = page
      .getByRole('combobox')
      .filter({ hasText: 'Disabled select' });
    await expect(disabledSelect).toHaveAttribute('disabled', '');
    await disabledSelect.click({ force: true });
    await expect(
      page.getByRole('option', { name: 'Disabled value 1' }),
    ).not.toBeVisible();
    await expect(
      page.getByRole('option', { name: 'Disabled value 2' }),
    ).not.toBeVisible();
  });
});
